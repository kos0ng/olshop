...
if(isset($_POST['submit'])){
  ...
  if($_POST['password']==''){
    ...
    $data = mysqli_query($koneksi,"UPDATE admin SET username='$username' WHERE id_admin='$id_admin'");
    ...
    echo '<script>alert("Password tidak sama");window.location.replace("detail-akun.php"); </script>';
       ...
       $_SESSION['username']=$username;
      $data = mysqli_query($koneksi,"UPDATE admin SET username='$username',password='$password' WHERE id_admin='$id_admin'");
    echo '<script>alert("Berhasil update!");window.location.replace("detail-akun.php"); </script>';
